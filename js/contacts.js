...
function addBlueBackground(i) {
    ...
    if (selectedContact === contactContainer) {
        // Entferne den blauen Hintergrund und fahre den Slider heraus, wenn derselbe Container erneut ausgewählt wird
        contactContainer.classList.remove('blue-background');
        document.getElementById('selected-container').classList.remove('show');
        selectedContact = null;
    } else {
        // Entferne den blauen Hintergrund vom vorherigen Container
        if (selectedContact !== null) {
            selectedContact.classList.remove('blue-background');
        }

        // Füge den blauen Hintergrund zum neuen Container hinzu
        contactContainer.classList.add('blue-background');

        // Slider sofort hereinfahren lassen
        document.getElementById('selected-container').classList.add('show');
        ...
        <div class="edit-contact_main-container">
            <div class="edit-area_top-area d-flex_column">
                <div class="close-container d-flex_column">
                    <div onclick="closeEditContactOverlay()" class="close-edit-contacts">
            ...
            <div class="edit-area_bottom-area d-flex_column">
                <form id="edit-contact-form" onsubmit="editContact(${i}); return false;">
                    <div class="name-edit-contacts d-flex_row input-container">
                    ...
                    <div class="email-edit-contacts d-flex_row input-container">
                        <input id="edit-email" name="email" placeholder="Email" class="email-input" type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" title="Bitte geben Sie eine gültige E-Mail-Adresse ein" value="${contacts[i]['email']}" required>
                    ...
                    <div class="phone-edit-contacts d-flex_row input-container">
                    ...
                    <div id="edit-button-container" class="button_container d-flex_row">
                        ...
                        <button type="submit" id="edit_in_edit" class="save-button-container d-flex_row">
...
/* async function editContact(i) {
    ...
    let { name: newName, email: newEmail, mobile: newMobile } = newInput;

    // Validate name and email fields
    if (!newName || !newEmail) {
        console.error("Name und Email dürfen nicht leer sein.");
        return;
    }

    // Check if the mobile field should be validated
    let isMobileChange = false;
    if (contact.mobile) {
        if (!newMobile) {
            console.error("Telefonnummer darf nicht leer sein.");
            return;
        }
        isMobileChange = contact.mobile !== newMobile;
    } else {
        isMobileChange = newMobile !== '';
    }

    // Check if there are any changes
    if (contact.name === newName && contact.email === newEmail && !isMobileChange) {
    ...
    let confirmationMessage = `
        ...
        \nMobil: ${contact.mobile || "nicht angegeben"} zu ${newMobile || "nicht angegeben"}
    `;
